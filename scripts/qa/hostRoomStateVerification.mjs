...
async function readRemotePlayer(page) {
  return page.evaluate("window.__dustlineQa__?.getState()?.match?.remotePlayers?.[0] ?? null");
}
...
async function setInputState(page, movementX, movementZ, crouching = false, jumpRequested = false) {
  await page.evaluate(
    `window.__dustlineQa__.setInputState(${movementX}, ${movementZ}, ${JSON.stringify(crouching)}, ${JSON.stringify(jumpRequested)})`,
  );
}
...
async function sendInputTick(
  ...
  return page.evaluate(
    `window.__dustlineQa__.sendInputTick(${movementX}, ${movementZ}, ${JSON.stringify(crouching)}, ${JSON.stringify(jumpRequested)})`,
  );
}
...
async function readLocalPlayer(page) {
  return page.evaluate("window.__dustlineQa__?.getState()?.match?.localPlayer ?? null");
}

async function waitForPositionConvergence(
  readFirstPosition,
  readSecondPosition,
  threshold = 0.35,
  timeoutMs = 15_000,
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const [first, second] = await Promise.all([readFirstPosition(), readSecondPosition()]);

    if (first && second) {
      const error = Math.hypot(first.x - second.x, first.z - second.z);
      if (error < best.error) {
        best = { first, second, error };
      }
      if (error < threshold) {
  ...
  return page.evaluate(
    `(() => {
      ...
      const inputMatches =
        ...
        Boolean(remote.inputCrouching) === ${JSON.stringify(crouching)};
...
async function driveGuestInputUntilObserved(
  ...
  await setInputState(joinPage, movementX, movementZ, crouching);
  await hostPage.bringToFront();

  while (Date.now() - startedAt < timeoutMs) {
    await sendInputTick(joinPage, movementX, movementZ, crouching);
...
async function main() {
  ...
  try {
    ...
    const jitterConvergence = await waitForPositionConvergence(
      () => readLocalPosition(joinPage),
      () => readRemotePosition(hostPage),
    );
    const guestJitterSettledState = await joinPage.evaluate(
      "window.__dustlineQa__?.getState()?.match?.localPlayer ?? null",
    );
    const guestLocalAfterJitterSettle = jitterConvergence.first;
    const hostGuestAfterJitter = jitterConvergence.second;
    assert(
      guestLocalAfterJitterSettle && hostGuestAfterJitter,
      "Positions were unavailable after the delayed-snapshot jitter test.",
    );
    const guestJitterConvergenceError = jitterConvergence.error;
    assert(
    ...
    await joinPage.waitForExpression(
      `(() => {
        const local = window.__dustlineQa__?.getState()?.match?.localPlayer;
        return Boolean(local) && local.grounded === true && local.position?.y >= 1.45;
      })()`,
      10_000,
    );
    const guestJumpStartState = await readLocalPlayer(joinPage);
    assert(guestJumpStartState?.grounded === true, "Guest was not grounded before the shared jump test.");
    const jumpDelayConfigured = await configureLatestStateQa(joinPage, "inbound", {
      delayScheduleMs: [650, 280, 180, 120, 80],
    });
    assert(jumpDelayConfigured === true, "Could not enable inbound latest-state delay for the jump test.");
    await setInputState(joinPage, 0, 0, false, true);
    await joinPage.bringToFront();
    const jumpSent = await sendInputTick(joinPage, 0, 0, false, true);
    assert(jumpSent === true, "Guest jump input tick was not sent.");

    await hostPage.bringToFront();
    await hostPage.waitForExpression(
      `(() => {
        const remote = window.__dustlineQa__?.getState()?.match?.remotePlayers?.[0];
        return Boolean(remote) && remote.movement?.grounded === false && (remote.position?.y ?? 0) > 0.12;
      })()`,
      10_000,
    );
    const hostRemoteDuringJump = await readRemotePlayer(hostPage);

    await joinPage.bringToFront();
    await joinPage.waitForExpression(
      `(() => {
        const local = window.__dustlineQa__?.getState()?.match?.localPlayer;
        return Boolean(local) && local.grounded === false && (local.position?.y ?? 0) > ${
          (guestJumpStartState?.position?.y ?? 1.62) + 0.18
        };
      })()`,
      5_000,
    );
    const guestJumpMidState = await readLocalPlayer(joinPage);
    const guestJumpHeightRise =
      (guestJumpMidState?.position?.y ?? 0) - (guestJumpStartState?.position?.y ?? 0);
    assert(
      guestJumpHeightRise > 0.18,
      `Guest local jump prediction only gained ${guestJumpHeightRise.toFixed(3)}m of height.`,
    );
    assert(
      (guestJumpMidState?.lastSentInputSequence ?? 0) >
        (guestJumpMidState?.lastAcknowledgedInputSequence ?? 0),
      `Guest jump prediction did not retain an ack gap under delayed snapshots: ${JSON.stringify(guestJumpMidState)}`,
    );

    await clearInputState(joinPage);
    await hostPage.bringToFront();
    await delay(150);
    const hostProcessedSequenceAfterJump = await hostPage.evaluate(
      "window.__dustlineQa__?.getState()?.match?.remotePlayers?.[0]?.lastProcessedInputSequence ?? 0",
    );
    await joinPage.bringToFront();
    await joinPage.waitForExpression(
      `(() => {
        ...
        return Boolean(local) &&
          local.grounded === true &&
          local.lastAcknowledgedInputSequence >= ${hostProcessedSequenceAfterJump};
      })()`,
      15_000,
    );
    await hostPage.bringToFront();
    await hostPage.waitForExpression(
      `(() => {
        const remote = window.__dustlineQa__?.getState()?.match?.remotePlayers?.[0];
        return Boolean(remote) && remote.movement?.grounded === true && Math.abs(remote.position?.y ?? 0) < 0.05;
      })()`,
      10_000,
    );
    const guestJumpSettledState = await readLocalPlayer(joinPage);
    const hostRemoteAfterJump = await readRemotePlayer(hostPage);
    assert(guestJumpSettledState && hostRemoteAfterJump, "Jump reconciliation states were unavailable.");
    const hostRemoteEyeY =
      (hostRemoteAfterJump?.position?.y ?? 0) + (hostRemoteAfterJump?.movement?.eyeHeight ?? 0);
    const guestJumpEyeError = Math.abs((guestJumpSettledState?.position?.y ?? 0) - hostRemoteEyeY);
    const guestJumpConvergenceError = Math.hypot(
      (guestJumpSettledState?.position?.x ?? 0) - (hostRemoteAfterJump?.position?.x ?? 0),
      (guestJumpSettledState?.position?.z ?? 0) - (hostRemoteAfterJump?.position?.z ?? 0),
    );
    assert(
      guestJumpEyeError < 0.12,
      `Guest jump landing eye height diverged from host authority by ${guestJumpEyeError.toFixed(3)}m.`,
    );
    assert(
      guestJumpConvergenceError < 0.35,
      `Guest jump landing xz reconciliation diverged by ${guestJumpConvergenceError.toFixed(3)}m.`,
    );
    await configureLatestStateQa(joinPage, "inbound", null);
    ...
    const summary = {
      ...
      guestJumpStartState,
      hostRemoteDuringJump,
      guestJumpMidState,
      guestJumpHeightRise,
      guestJumpSettledState,
      hostRemoteAfterJump,
      guestJumpEyeError,
      guestJumpConvergenceError,
