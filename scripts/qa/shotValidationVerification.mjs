...
async function main() {
  ...
  try {
    ...
    const blockedGuestAfterClaim = await readMatch(joinPage);
    assert(
      (blockedGuestAfterClaim?.shots?.events ?? []).some((event) => event.type === "local"),
      "Guest shot claim did not produce a local shot event.",
    );
    assert(
      (blockedGuestAfterClaim?.shots?.events ?? []).some((event) => event.type === "shared-sent"),
      "Guest shot claim did not produce a shared-sent event.",
    );
    assert(
      (blockedGuestAfterClaim?.audio ?? []).some((event) => event.type === "local-fire"),
      "Guest shot claim did not produce local-fire audio feedback.",
    );
    assert(
      blockedGuestAfterClaim?.weaponView?.muzzleFlashRecent === true,
      "Guest shot claim did not leave recent muzzle-flash evidence.",
    );
    assert(
      (blockedGuestAfterClaim?.localPlayer?.ammoInClip ?? 24) < 24,
      "Guest provisional shot did not consume local ammo before host authority replied.",
    );
    ...
    await delay(250);
    const ammoBeforeCount = await currentResultCount(joinPage);
    const ammoSent = await joinPage.evaluate(
      `window.__dustlineQa__.submitShotClaim({
        ammoInClip: 99,
        origin: ${JSON.stringify(clearLayout.guest)},
        direction: {
    ...
    assert(ammoSent === true, "Guest could not submit the forged ammo shot claim.");
    const [ammoResult] = await waitForNewResults(joinPage, ammoBeforeCount, 1);
    const hostHealthAfterAmmo = await hostPage.evaluate(
      "window.__dustlineQa__?.getState()?.match?.localPlayer?.health ?? -1",
    );
    const guestAmmoAfterAmmo = await joinPage.evaluate(
      "window.__dustlineQa__?.getState()?.match?.localPlayer?.ammoInClip ?? -1",
    );
    assert(ammoResult?.reason === "ammo-state", `Ammo-state result was ${JSON.stringify(ammoResult)}`);
    assert(hostHealthAfterAmmo === 66, `Forged ammo claim changed host health to ${hostHealthAfterAmmo}`);
    assert(
      guestAmmoAfterAmmo === guestAmmoAfterAccepted,
      `Forged ammo claim changed guest ammo from ${guestAmmoAfterAccepted} to ${guestAmmoAfterAmmo}`,
    );

    await delay(250);
    const reloadBeforeCount = await currentResultCount(joinPage);
    const reloadSent = await joinPage.evaluate(
      `window.__dustlineQa__.submitShotClaim({
        reloadSequence: 99,
        origin: ${JSON.stringify(clearLayout.guest)},
        direction: {
    ...
    assert(reloadSent === true, "Guest could not submit the forged reload shot claim.");
    const [reloadResult] = await waitForNewResults(joinPage, reloadBeforeCount, 1);
    const hostHealthAfterReload = await hostPage.evaluate(
      "window.__dustlineQa__?.getState()?.match?.localPlayer?.health ?? -1",
    );
    const guestAmmoAfterReload = await joinPage.evaluate(
      "window.__dustlineQa__?.getState()?.match?.localPlayer?.ammoInClip ?? -1",
    );
    assert(
      reloadResult?.reason === "reload-state",
      `Reload-state result was ${JSON.stringify(reloadResult)}`,
    );
    assert(
      hostHealthAfterReload === 66,
      `Forged reload claim changed host health to ${hostHealthAfterReload}`,
    );
    assert(
      guestAmmoAfterReload === guestAmmoAfterAccepted,
    ...
    const summary = {
      ...
      blockedLayout,
      blockedGuestAfterClaim: {
        shots: blockedGuestAfterClaim?.shots,
        audio: blockedGuestAfterClaim?.audio,
        muzzleFlashRecent: blockedGuestAfterClaim?.weaponView?.muzzleFlashRecent,
        ammoInClip: blockedGuestAfterClaim?.localPlayer?.ammoInClip,
      },
      blockedResult,
      clearLayout,
      acceptedResult,
